      ...
      <div className="flex overflow-x-auto border-t border-[#1E2A3B] h-7" style={{scrollbarWidth:"none"}}>
        {ticks.map((t,i) => (
          <div key={i} className="flex items-center gap-1.5 px-3 h-full border-r border-[#1E2A3B] flex-shrink-0">
            <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:"9px",letterSpacing:"0.1em"}} className="text-[#334155] uppercase">{t.label}</span>
            <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:"11px"}} className="text-[#CBD5E1]">{t.value}</span>
            {t.chg!=null&&<span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:"10px"}} className={t.chg>=0?"text-emerald-400":"text-red-400"}>{t.chg>=0?"+":""}{t.chg.toFixed(1)}%</span>}
          </div>
        ))}
      </div>
    </header>
...
function LiveClock() {
  ...
  return <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:"10px"}} className="text-[#334155] hidden sm:inline">{time}</span>;
}
